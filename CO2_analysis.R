...
df_attractant <- read.csv("Attractant_data.csv", sep =";", h = T)
...
comp_df <- read.csv("Yeast_canister_data.csv", sep =";", h = T)

comp_df <- comp_df %>% 
  mutate_at(c("Site", "TrapType",
              "Date", "Species", "Family"), as.factor)


#Compare capture rates across species
comp_mod <- glmmTMB(
  CountPerHour ~ TrapType * Species + (1|Date) + (1|Site),
  family = Gamma(link = "log"),
  data = comp_df
)
...
emmeans(comp_mod, pairwise ~ TrapType | Species, 
                type = "response", adjust = "none")

plot_pred_comp_sp <- as.data.frame(emmeans(comp_mod, ~ TrapType | Species, 
                                   type = "response", adjust = "none"))
...
predicted_comp_sp <- ggplot() +
  geom_jitter(data = comp_df, aes(x = TrapType, y = CountPerHour, color = TrapType), alpha = 0.5,
              width = 0.1) +
  geom_errorbar(
  ...
  geom_point(data = plot_pred_comp_sp, aes(x = TrapType, y = response, color = TrapType), size = 4) +
  theme_classic() +
  ylab("Individuals per hour") +
  facet_wrap(~ Species, scales = "free_y") +
  scale_color_manual(values = c("#4590ba", "#b87f00")) +
...
Fam_df <- comp_df %>% 
  group_by(Family, Date, TrapType, Site) %>% 
  summarize(
    Sum = sum(CountPerHour)
  )

Fam_comp_mod <- glmmTMB(
  Sum ~ TTrapType * Family + (1|Date) + (1|Site),
  family = Gamma(link = "log"),
  data = Fam_df
)
...
#Pairwise comparisons
emmeans(Fam_comp_mod, pairwise ~ TrapType | Family, 
                type = "response", adjust = "none")

Fam_comp_pred <- as.data.frame(emmeans(Fam_comp_mod, ~ TrapType | Family, 
                            type = "response", adjust = "none"))

predicted_comp_fam <- ggplot() +
  geom_jitter(data = Fam_df, aes(x = TrapType, y = Sum, color = TrapType), alpha = 0.5,
              width = 0.1) +
  geom_errorbar(
    data = Fam_comp_pred,
    aes(
  ...
  geom_point(data = Fam_comp_pred, aes(x = TrapType, y = response, color = TrapType), size = 4) +
  theme_classic() +
  ylab("Individuals per hour") +
  facet_wrap(~ Family, scales = "free_y") +
  scale_color_manual(values = c("#4590ba", "#b87f00")) +
...
ggsave(comparison_plots, filename = "Fig4_CO2.TIFF", dpi = 450,
